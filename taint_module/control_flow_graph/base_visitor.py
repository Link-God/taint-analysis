...
from taint_module.helpers.cf_graph_helpers.base_visitor_helper import (
    CALL_IDENTIFIER,
    ConnectStatements,
    connect_nodes,
...
from taint_module.helpers.core_helpers.ast_helper import generate_ast, get_call_names, get_call_names_as_string

from ..core.module_definitions import LocalModuleDefinition, ModuleDefinition, ModuleDefinitions
from ..core.node_types import (
    AssignmentCallNode,
    AssignmentNode,
    BlackBoxOrBuiltInNode,
    BreakNode,
    ControlFlowNode,
    EntryOrExitNode,
    IfNode,
...
from ..core.project_handler import get_directory_modules
from ..helpers.cf_graph_helpers.alias_helper import (
    as_alias_handler,
...
from ..helpers.visit_helper.label_visitor import LabelVisitor
...
class BaseVisitor(ast.NodeVisitor):
    def __init__(self, project_modules, local_modules, filename: str, allow_local_directory_imports: bool = True):
        self._allow_local_modules: bool = allow_local_directory_imports
        self.project_modules: list = project_modules
        self.local_modules = local_modules if self._allow_local_modules else []
        self.prev_nodes_to_avoid: list[Node] = []
        self.last_control_flow_nodes: list[Optional[Node]] = []
        self.nodes: list[Node] = []
        self.module_definitions_stack: list[ModuleDefinitions] = []
        self.filenames: list[str] = [filename]
        self.function_names: list[str] = []
        self.function_return_stack: list[str] = []
        self.function_call_index: int = 0
        self.blackbox_assignments: set[BlackBoxOrBuiltInNode] = set()
        super().__init__()

    def visit_Module(self, node):
        return self.start_handler(node.body)
    ...
    def visit_If(self, node):
        ...
        body_connect_stmts = self.start_handler(node.body)
    ...
    def visit_Return(self, node: ast.Return):
        ...
        return self.append_node(
            ReturnNode(lhs + " = " + label.result, lhs, node, rhs_visitor_result, path=self.filenames[-1])
        )

    def visit_Try(self, node):
        try_node = self.append_node(TryNode(node, path=self.filenames[-1]))
        body = self.start_handler(node.body)
        ...
        for handler in node.handlers:
            ...
            handler_node = self.append_node(
                Node("except " + name + ":", handler, line_number=handler.lineno, path=self.filenames[-1])
            )
            for body_node in body.last_statements:
                body_node.connect(handler_node)
            handler_body = self.start_handler(handler.body)
        ...
        if node.orelse:
            or_else_last_nodes = self.handle_or_else(node.orelse, body.last_statements[-1])
            body.last_statements.extend(or_else_last_nodes)

        if node.finalbody:
            final_body = self.start_handler(node.finalbody)
            for last in last_statements:
                last.connect(final_body.first_statement)

            for last in body.last_statements:
                last.connect(final_body.first_statement)

            body.last_statements.extend(final_body.last_statements)
    ...
    def visit_Assign(self, node):
        ...
        if isinstance(node.targets[0], (ast.Tuple, ast.List)):  # x,y = [1,2]
            if isinstance(node.value, (ast.Tuple, ast.List)):
                ...
                return self.append_node(
                    AssignmentNode(label.result, label.result, node, rhs_visitor.result, path=self.filenames[-1])
                )
    ...
    def visit_AugAssign(self, node):
        ...
        return self.append_node(
            AssignmentNode(label.result, lhs, node, rhs_visitor.result + [lhs], path=self.filenames[-1])
        )
    ...
    def visit_For(self, node):
        ...
        for_node = self.append_node(
            Node("for " + target_label.result + " in " + iterator_label.result + ":", node, path=self.filenames[-1])
        )
    ...
    def visit_With(self, node):
        ...
        with_node = self.append_node(Node(label_visitor.result, node, path=self.filenames[-1]))
        connect_statements = self.start_handler(node.body)
    ...
    def visit_Delete(self, node):
        ...
        return self.append_node(Node("del " + labelVisitor.result, node, path=self.filenames[-1]))

    def visit_Assert(self, node):
        label_visitor = LabelVisitor()
        label_visitor.visit(node.test)

        return self.append_node(Node(label_visitor.result, node, path=self.filenames[-1]))
            ...
            for module in self.local_modules:
                if name.name == module[0]:
                    if os.path.isdir(module[1]):
                        return self.import_package(module, name, name.asname, retrieve_import_alias_mapping(node.names))
    ...
    def visit_ImportFrom(self, node):
            ...
            if node.module == module[0]:
                if os.path.isdir(module[1]):
                    return self.from_directory_import(
                        module, not_as_alias_handler(node.names), as_alias_handler(node.names)
                    )
                return self.add_module(
                    module=module,
        ...
        for name in node.names:
            local_definitions = self.module_definitions_stack[-1]
            local_definitions.import_alias_mapping[name.asname or name.name] = f"{node.module}.{name.name}"
    ...
    def visit_While(self, node):
        ...
        while_node = self.append_node(Node("while " + label_visitor.result + ":", node, path=self.filenames[-1]))
    ...
    def start_handler(self, stmts, prev_node_to_avoid=None):
        ...
        if cfg_statements:
            ...
            return ConnectStatements(
                first_statement=first_statement, last_statements=last_statements, break_statements=break_nodes
            )
    ...
    def add_to_definitions(self, node):
        ...
        if parent_definitions:
            ...
            parent_definition = ModuleDefinition(
                parent_definitions, parent_qualified_name, local_definitions.module_name, self.filenames[-1]
            )
    ...
    def handle_or_else(self, or_else: list[ast.stmt], test):
        """
        Handle the or_else part of an if or try node.

        Returns:
            The last nodes of the orelse branch.
        """
        if isinstance(or_else[0], ast.If):
            control_flow_node = self.visit(or_else[0])
            ...
            else_connect_statements = self.start_handler(or_else, prev_node_to_avoid=self.nodes[-1])
    ...
    def handle_stmt_star_ignore_node(body, fallback_cfg_node):
        try:
            ...
            body = ConnectStatements(
                first_statement=[fallback_cfg_node], last_statements=[fallback_cfg_node], break_statements=[]
            )
        return body
    ...
    def loop_node_skeleton(self, test, node):
        ...
        body_connect_stmts = self.start_handler(node.body, prev_node_to_avoid=self.nodes[-1])
        ...
        if node.orelse:
            orelse_connect_stmts = self.start_handler(node.orelse, prev_node_to_avoid=self.nodes[-1])
    ...
    def add_blackbox_or_builtin_call(self, node, blackbox):  # noqa: C901
        ...
        Returns:
            call_node(BlackBoxOrBuiltInNode): The call node.
        """
        ...
        call_node = BlackBoxOrBuiltInNode(
            label="",
            left_hand_side=lhs,
            ast_node=node,
            right_hand_side_variables=[],
        ...
        for arg_node in itertools.chain(node.args, node.keywords):
            ...
            if isinstance(arg, ast.Call):
                ...
                vars_visitor.visit(arg_node)
                rhs_vars.extend(vars_visitor.result)
        if last_return_value_of_nested_call:
    ...
    def add_module(  # noqa: C901
        ...
        if new_module_definitions.is_init:
            for def_ in new_module_definitions.definitions:
                module_def_alias = handle_aliases_in_init_files(def_.name, new_module_definitions.import_alias_mapping)
                parent_def_alias = handle_aliases_in_init_files(def_.name, parent_definitions.import_alias_mapping)
                ...
                if module_or_package_name:
                    if from_from:
                        ...
                        if from_fdid:
                            ...
                            parent_definition = ModuleDefinition(
                                parent_definitions, qualified_name, module_or_package_name, self.filenames[-1]
                            )
                        else:
                            parent_definition = ModuleDefinition(
                                parent_definitions, qualified_name, None, self.filenames[-1]
                            )
                    else:
                        qualified_name = module_or_package_name + "." + def_name
                        parent_definition = ModuleDefinition(
                            parent_definitions, qualified_name, parent_definitions.module_name, self.filenames[-1]
                        )
                    parent_definition.node = def_.node
                    parent_definitions.definitions.append(parent_definition)
                else:
                    parent_definition = ModuleDefinition(
                        parent_definitions, def_name, parent_definitions.module_name, self.filenames[-1]
                    )
                    parent_definition.node = def_.node
                    parent_definitions.definitions.append(parent_definition)
