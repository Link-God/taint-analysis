...
from taint_module.helpers.cf_graph_helpers.base_visitor_helper import CALL_IDENTIFIER
from taint_module.helpers.cf_graph_helpers.expr_visitor_helper import (
    BUILTINS,
    MUTATORS,
    SavedVariable,
    return_connection_handler,
)
from taint_module.helpers.core_helpers.ast_helper import Arguments, get_call_names_as_string
...
from ..helpers.cf_graph_helpers.alias_helper import handle_aliases_in_calls
...
class ExprVisitor(BaseVisitor):
    def __init__(
        ...
        super().__init__(
            ...
            filename=filename,
            allow_local_directory_imports=allow_local_directory_imports,
        )
    ...
    def init_function_cfg(self, node, module_definitions):
        ...
        module_statements = self.start_handler(node.body)
    ...
    def visit_Yield(self, node):
        ...
        return self.append_node(
            YieldNode(LHS + " += " + label.result, LHS, node, rhs_visitor_result + [LHS], path=self.filenames[-1])
        )
    ...
    def save_local_scope(self, line_number, saved_function_call_index):
        ...
        for assignment in [
            ...
            if (type(node) == AssignmentNode or type(node) == AssignmentCallNode or type(Node) == BlackBoxOrBuiltInNode)
        ...
        for i, call_arg in enumerate(call_args):
            ...
            if isinstance(call_arg, ast.Call):
                if last_return_value_of_nested_call:
                    ...
                    if isinstance(return_value_of_nested_call, BlackBoxOrBuiltInNode):
    ...
    def visit_and_get_function_nodes(self, definition, first_node):
        ...
        function_body_connect_statements = self.start_handler(definition.node.body)
    ...
    def restore_saved_local_scope(self, saved_variables, args_mapping, line_number):
        ...
        for var in saved_variables:
            ...
            if var.RHS in args_mapping:
                ...
                restore_nodes.append(
                    RestoreNode(
                        var.RHS + " = " + var.LHS, var.RHS, [var.LHS], line_number=line_number, path=self.filenames[-1]
                    )
                )
    ...
    def process_function(self, call_node, definition):
        ...
        args_mapping, first_node = self.save_def_args_in_temp(
            call_node.args, Arguments(def_node.args), call_node.lineno, saved_function_call_index, first_node
        )
        self.filenames.append(definition.path)
        self.create_local_scope_from_def_args(
            call_node.args, Arguments(def_node.args), def_node.lineno, saved_function_call_index
        )
        ...
        self.return_handler(call_node, function_nodes, saved_function_call_index, first_node)
    ...
    def visit_Call(self, node):
        ...
        elif not self._within_mutating_call and last_attribute in MUTATORS and isinstance(node.func, ast.Attribute):
