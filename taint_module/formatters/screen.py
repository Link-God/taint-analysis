...
def vulnerability_to_str(i, vulnerability):
    ...
    if vulnerability.reassignment_nodes:
        ...
        for node in vulnerability.reassignment_nodes:
            ...
            label = node.label
            if isinstance(vulnerability, SanitisedVulnerability) and node.label == vulnerability.sanitiser.label:
    ...
    lines.append("\t{}".format(color(vulnerability.sink.label, RED_ON_WHITE)))
