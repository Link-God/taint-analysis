...
from taint_module.analysis import ReachingDefinitionsTaintAnalysis, constraint_table
from taint_module.control_flow_graph import ControlFlowGraph
from taint_module.core.project_handler import get_directory_modules, get_modules
from taint_module.helpers.core_helpers.ast_helper import generate_ast
...
from taint_module.helpers.web_frameworks_helper.framework_helper import (
    is_django_view_function,
    is_flask_route_function,
    is_function,
    is_function_without_leading_,
)
from taint_module.usage import parse_args
from taint_module.vulnerabilities.vulnerabilities import find_vulnerabilities
from taint_module.web_frameworks import FrameworkAdaptor
...
def retrieve_nosec_lines(path):
    ...
    return set(lineno for (lineno, line) in enumerate(lines, start=1) if "#nosec" in line or "# nosec" in line)
...
def main(command_line_args=sys.argv[1:]):  # noqa: C901
    ...
    for path in sorted(files):
        ...
        if args.adaptor:
            ...
            elif args.adaptor.lower().startswith("f"):
    ...
    log.info("Analysing")
    ReachingDefinitionsTaintAnalysis.analyse(cfg_list)
    log.info("Finding vulnerabilities")
    vulnerabilities = find_vulnerabilities(
        cfg_list, args.blackbox_mapping_file, args.trigger_word_file, args.interactive, nosec_lines
    )
    ...
    has_unsanitised_vulnerabilities = any(not isinstance(v, SanitisedVulnerability) for v in vulnerabilities)
