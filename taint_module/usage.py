...
default_trigger_word_file = os.path.join(
    os.path.dirname(__file__), "vulnerability_definitions", "all_trigger_words.json"
)
...
def _add_required_group(parser):
    ...
    required_group.add_argument(
        "targets", metavar="targets", nargs="+", help="source file(s) or directory(s) to be scanned", type=str
    )
...
def _add_optional_group(parser):
    ...
    optional_group.add_argument(
        "-a", "--adaptor", help="Choose a web framework adaptor: " "Flask(Default), Django, Every or Pylons", type=str
    )
    optional_group.add_argument(
        "-pr",
        "--project-root",
        help="Add project root, only important when the entry " "file is not at the root of the project.",
        type=str,
    )
    optional_group.add_argument(
        "-b",
    ...
    optional_group.add_argument(
        "--ignore-nosec", dest="ignore_nosec", action="store_true", help="Do not skip lines with # nosec comments"
    )
    optional_group.add_argument(
        "-r", "--recursive", dest="recursive", action="store_true", help="Find and process files in subdirectories"
    )
    optional_group.add_argument(
        "-x", "--exclude", dest="excluded_paths", action="store", default="", help="Separate files with commas"
    )
    optional_group.add_argument(
        "--dont-prepend-root",
...
def parse_args(args):
    if len(args) == 0:
        args.append("-h")
    parser = argparse.ArgumentParser(prog="python -m taint_module")
